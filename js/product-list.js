class ProductList {
    ...
    createProductDomString(product) {
        return `<article class="card col-12 col-sm-6 col-md-4 col-lg-3">
                ...
                <div class="card-body">
                    ...
                    <a href="#" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#product-info-modal" data-id=${product.id}>Детальніше</a>
                    <a href="#" class="btn btn-primary btn-buy" data-id=${product.id}> ${product.price} Купити</a>
                </div>
        </article>`;
    }
    addEventListeners() {
    ...
    addProductToCart(event) {
        ...
        const cart = new Cart();
        cart.addProduct(id);
        window.showAlert('Додано в кошик!');
    }
}
new ProductList();
