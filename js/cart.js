class Cart {
  ...
  async order(ev) {
    if (Object.keys(this.cart).length === 0) return window.showAlert("Виберіть товари для замовлення", false);
    const form = document.querySelector(".form-contacts");
    if (!form.checkValidity()) return window.showAlert("Будь ласка, заповніть форму правильно", false);
    ev.preventDefault();
    ...
    data.append("card", form.querySelector("input[name=card]").value);
    ...
    .then(() => {
      ...
      this.saveCart();
      this.renderCart();
      window.showAlert("Дякую за покупку " );
      document.querySelector("#modal-cart .close-btn").click();
    })
      .catch((error) => window.showAlert("Є помилка: " + error, false) ); 
  }
}
new Cart();
